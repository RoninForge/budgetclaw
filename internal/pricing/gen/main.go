...
func main() {
	...
	dataModified, err := parseDataModified()
	if err != nil {
		fatalf("read dataModified: %v", err)
	}

	src, err := renderTable(tag, prov.Commit, dataModified, series, aliases)
	if err != nil {
		fatalf("render: %v", err)
	}
...
// parseDataModified reads the vendored index.json's dataModified date.
//
// This is the dataset's own data date, derived upstream from the latest
// price validation, NOT a build timestamp. It is what lets the binary
// report how old its prices are, which is the difference between "this
// table is 40 days old" and "this binary was compiled 40 days ago".
func parseDataModified() (string, error) {
	raw, err := os.ReadFile(filepath.Join(indexDirRel, "index.json"))
	if err != nil {
		return "", err
	}
	var wrapper struct {
	...
	if wrapper.DataModified == "" {
		return "", fmt.Errorf("index.json has no dataModified")
	}
	if _, err := time.Parse("2006-01-02", wrapper.DataModified); err != nil {
		return "", fmt.Errorf("dataModified %q is not a YYYY-MM-DD date: %w", wrapper.DataModified, err)
	}
	return wrapper.DataModified, nil
}
...
func renderTable(tag, commit, dataModified string, series map[string]genModel, aliases map[string]string) ([]byte, error) {
	var b bytes.Buffer
	...
	// The dataset's own data date, so the binary can report how stale its
	// prices are without conflating that with its build date.
	fmt.Fprintf(&b, "const generatedDataModified = %q\n\n", dataModified)
