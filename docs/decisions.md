...
## 16. Pricing source: vendored, pinned ai-price-index with point-in-time rates and committed codegen (v1.0.0)

**Decision:** As of v1.0.0, the pricing table is generated from a VENDORED, pinned release of the public [ai-price-index](https://github.com/RoninForge/ai-price-index) dataset rather than a hand-maintained `baseRates` map. The anthropic artifacts are committed under `internal/pricing/index/**` with a `PROVENANCE.json` sha256 manifest, and `internal/pricing/gen/main.go` code-generates `internal/pricing/table_gen.go` from them. The dataset records POINT-IN-TIME pricing (per-model half-open `[from, to)` input and output intervals), so an event is priced at the rate effective on its own timestamp. Cache multipliers stay engine constants in `pricing.go`. The generated `table_gen.go` is committed.

**Alternatives:** keep the hand-maintained `baseRates` map (decision 15i); fetch the dataset at runtime; fetch + generate at build time without committing the output.

**Reason:**
- **Single source of truth.** The same dataset drives the budgetclaw binary, the Goei dashboard, and the roninforge.org landing pricing tool. A hand-maintained table in each would drift; one vendored dataset with shared golden vectors (`internal/pricing/index/pricing-vectors.json`) keeps every engine in agreement.
- **Point-in-time is correct, not just nice.** Costs are frozen historical fact (decision 15e). A flat "current rate" table re-prices yesterday's events at today's rate on every `backfill --rebuild`, silently rewriting history when Anthropic cuts a tier. Per-event timestamp resolution prices each event at what the user actually paid.
- **Vendored + committed = hermetic, offline, auditable.** The trust pledge requires zero runtime network access (AGENTS.md). Vendoring the artifacts and committing the generated table means `go build` never touches the network, the binary's rates are reproducible from a pinned commit, and an auditor can trace any rate to an exact upstream commit via `budgetclaw pricing provenance`. A CI freshness gate (`go generate ./...` + `git diff --exit-code`) fails if the committed table drifts from the vendored data.

**Refresh loop** (replaces "edit `baseRates`" from decision 15i): bump `internal/pricing/PINNED_TAG` to the new dataset tag, run `go run ./internal/pricing/gen -fetch` (refreshes the vendored artifacts + `PROVENANCE.json` from the pinned tag and regenerates `table_gen.go`), then commit. The arithmetic, parity, and golden-vector tests guard against silent drift.
